    ...
    <div class="account container py-5">
        <div class="row">
            ...
            <div class="col-lg-9 ">
                ...
                <form class="mt-3 account_panel" method="POST" action="{{route('profile.edit')}}">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-6 mb-3">
                            <label class="font-size-xs text-uppercase font-weight-medium text-dark text-ls mb-0">{{__('firstName_title')}}</label>
                            <input type="text" class="form-control  mt-2" id="firstName" name="first_name" value="{{auth()->user()->first_name}}">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="font-size-xs text-uppercase font-weight-medium text-dark text-ls mb-0">{{__('lastName_title')}}</label>
                            <input type="text" class="form-control  mt-2" id="lastName" name="last_name" value="{{auth()->user()->last_name}}">
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="email" class="form-control  mt-2" id="email" name="email" value="{{auth()->user()->email}}">
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="font-size-xs text-uppercase font-weight-medium text-dark text-ls mb-0">{{__('Phone_title')}}</label>
                            <input type="tel" class="form-control mt-2 tel" id="phone"  name="phone" value="{{auth()->user()->phone}}">
                        </div>
