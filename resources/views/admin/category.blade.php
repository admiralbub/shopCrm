...
<div class="treeCategory">
    <ul class="tree treeSort1 treeSort" id="tree" data-parent="">
        @foreach($categories->sortBy('sort') as $category)

            <li data-id="{{$category->id}}" class="px-2">
                <details>
                    <summary class="d-flex justify-content-between">
                        {{$category->name_ua}}
                        <a href="{{route('platform.category.edit', $category->id)}}" class="px-4">
                    ...
                    @foreach ($category->childrenCategories->sortBy('sort') as $childCategory)
                        <ul class="treeSort2 treeSort" data-parent="{{$childCategory->category_id}}">
