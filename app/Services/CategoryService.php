    ...
    class CategoryService implements CategoryInterface {
        ...
        public function selectedFilter($attrFilter) {
            ...
            if(!empty($attrFilter["brand"])) {
                $selectedFilter["brand"] = $attrFilter["brand"]  ? array_map('intval', $attrFilter["brand"]) : [];
            } else {
                $selectedFilter["attr_group"] = $attrFilter  ? array_map('intval', $attrFilter) : [];
            }
            
            return $selectedFilter;
        }
        public function setFilter($products,$filter) {
            $filter = str_replace('-'.explode('-', $filter)[1], '', $filter);
            if($filter == "brand") {
                ...
                $id_group = AttrGroup::where('slug',$filter)->first();
                return $products = $products->whereHas(
                    'attrs',
